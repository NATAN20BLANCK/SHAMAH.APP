...
if (Platform.OS !== 'web') {
  try {
    ...
    console.warn('expo-image-picker ou expo-media-library não disponível:', error);
  }
} else {
  // Fallback para web
  try {
    const ExpoImagePickerMock = require('../utils/ExpoImagePicker');
    const ExpoMediaLibraryMock = require('../utils/ExpoMediaLibrary');
    ImagePicker = ExpoImagePickerMock.default || ExpoImagePickerMock;
    MediaLibrary = ExpoMediaLibraryMock.default || ExpoMediaLibraryMock;
  } catch (error) {
    console.warn('Mocks não disponíveis, usando fallbacks:', error);
    // Fallback básico
    ImagePicker = {
    ...
    MediaLibrary = {
      ...
      getAssetsAsync: async () => ({ assets: [], totalCount: 0, endCursor: '', hasNextPage: false })
    };
  }
}
